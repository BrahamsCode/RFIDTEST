...
/**
 * Fuente: `sql/vistas-analiticas.sql`. Documento de referencia:
 * `docs/13-kpis-y-analitica.md`.
 *
 * Las 7 vistas se derivan de `tags` y `stock_movements`, así que no hay que
 * mantenerlas sincronizadas con nada: reflejan siempre el estado actual.
 * `mv_daily_stock` sí es materializada y se refresca de noche.
 *
 * `stock_as_of()` reconstruye el stock de una fecha pasada desde los
 * movimientos. Es la prueba de que `stock_movements` es la fuente de verdad
 * y `tags` solo una proyección (ADR-005), y la base del control de
 * integridad nocturno de `docs/05` §6.
 */
return new class extends Migration
{
    ...
    {
        DB::unprepared(<<<'SQL'
            DROP FUNCTION IF EXISTS detect_cloned_tags(TIMESTAMPTZ);
            DROP FUNCTION IF EXISTS stock_as_of(BIGINT, TIMESTAMPTZ);
            DROP FUNCTION IF EXISTS cycle_zone_performance(BIGINT);
            DROP MATERIALIZED VIEW IF EXISTS mv_daily_stock;
            DROP VIEW IF EXISTS v_device_health;
            DROP VIEW IF EXISTS v_replenishment_needed;
            DROP VIEW IF EXISTS v_stock_aging;
            DROP VIEW IF EXISTS v_shrinkage;
            DROP VIEW IF EXISTS v_inventory_accuracy;
